...
const SCOPE_OPTIONS: { value: CorporateScope; label: string; helper: string }[] = [
  {
    value: "sp500",
    label: "S&P 500",
...
function getLocationBucket(row: CorporateProfile, scope: CorporateScope): string {
  if (scope === "sp500") {
      ...
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-4 py-3">
        <div>
          ...
          <p className="text-xs text-slate-500">
            {scope === "sp500" ? "Filter-responsive headquarters map using stored company Lat/Long." : "Global headquarters map using stored or fallback coordinates."}
          </p>
        </div>
  ...
  const locations = useMemo(
    () =>
      [
        ...
        ...Array.from(
          new Set(
            normalizedRows.map((row) => getLocationBucket(row, scope)),
          ),
        ).sort(),
      ] as string[],
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div>
              ...
              <h1 className="text-2xl font-semibold tracking-tight text-slate-950">{scopeTitle(scope)}</h1>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <StatCard
            ...
            helper={scope === "sp500" ? "Detected HQ states" : "Detected HQ countries"}
            icon={<Globe2 size={20} />}
          />
        </section>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          ...
          <BarListCard
            ...
            subtitle={scope === "sp500" ? "USA headquarters distribution" : "Global headquarters distribution"}
            data={locationChart}
          />
        </section>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 md:grid-cols-[1fr_220px_220px]">
            ...
            >
              {locations.map((item) => (
                <option key={item} value={item}>
                  {item === "All" ? (scope === "sp500" ? "All states" : "All countries") : item}
                </option>
              ))}
            </select>
          </div>
