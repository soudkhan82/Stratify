...
function MacroComparisonTick({
  ...
  const labels: Record<string, [string, string]> = {
    ...
    "Government revenue": ["Govt.", "revenue"],
    "Government expenditure": ["Govt.", "expenditure"],
    "Primary balance": ["Primary", "balance"],
    "Public debt": ["Public", "debt"],
    "Revenue": ["Govt.", "revenue"],
    "Expenditure": ["Govt.", "expenditure"],
    "Debt": ["Public", "debt"],
  };
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <text
        ...
        fontWeight={700}
      >
        <tspan x={0}>{lines[0]}</tspan>
        {lines[1] ? <tspan x={0} dy={15}>{lines[1]}</tspan> : null}
      </text>
    </g>
  );
}
export default function MacroFinancePage() {
  ...
  const compareRows = useMemo(() => {
    ...
    const labels = ["Broad money", "Government revenue", "Government expenditure", "Primary balance", "Public debt"];
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] text-slate-900">
      {loading ? (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center bg-[#f6f8fb]/95 px-4 backdrop-blur-[2px]"
          role="status"
          aria-live="polite"
          aria-label="Loading Macro and Finance data"
        >
          <div className="w-full max-w-[390px] rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <div className="text-sm font-black text-slate-950">
                  Loading Macro &amp; Finance
                </div>
                <div className="mt-1 text-xs font-medium leading-5 text-slate-500">
                  Fetching monetary, fiscal and sovereign debt data for {countryName}...
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-2">
              ...
              <div className="h-2.5 w-[84%] animate-pulse rounded-full bg-slate-100" />
              <div className="h-2.5 w-[62%] animate-pulse rounded-full bg-slate-100" />
            </div>

            <div className="mt-4 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400">
              ...
              <Panel title="Macro comparison" subtitle={`${year} reference year · nearest prior observation where needed`}>
                <div className="h-[430px] px-2 pb-3 pt-4 sm:px-4">
                  {compareLoading ? (
                      ...
                      <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 shadow-sm">
                        ...
                        <div>
                          ...
                          <div className="mt-0.5 text-xs font-medium text-slate-500">
                            Preparing comparison indicators...
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={compareRows} margin={{ top: 10, right: 18, bottom: 12, left: 2 }}>
                        <CartesianGrid stroke="#e2e8f0" strokeDasharray="3 5" vertical={false} />
                        <XAxis dataKey="metric" tick={<MacroComparisonTick />} axisLine={false} tickLine={false} interval={0} tickMargin={8} height={62} />
